...
if [[ "${HAVE_SUDO}" -eq 1 ]]; then
  ...
  echo "Attaching image with sudo..."
  LOOP_DEV="$(sudo losetup --find --show --partscan "${IMAGE_RAW}")"

  sudo mount -o ro "${LOOP_DEV}p1" "${BOOT_MNT}"
  sudo mount -o ro "${LOOP_DEV}p2" "${ROOT_MNT}"

  echo "Copying TFTP boot files..."
  sudo rsync -a --delete "${BOOT_MNT}/" "${TFTP_DIR}/"
else
...
if [[ "${HAVE_SUDO}" -eq 1 ]]; then
  sudo chown -R "$(id -u):$(id -g)" "${TFTP_DIR}"
else
  docker run --rm \
    -e HOST_UID="$(id -u)" \
    -e HOST_GID="$(id -g)" \
...
if [[ "${HAVE_SUDO}" -eq 1 ]]; then
  ...
  sudo mkdir -p "${NFS_ROOT}/boot/firmware"
  sudo rsync -a "${TFTP_DIR}/" "${NFS_ROOT}/boot/firmware/"
else
  docker run --rm \
    -v "${ROOT_DIR}:/work" \
    debian:bookworm-slim \
...
if [[ "${HAVE_SUDO}" -eq 1 ]]; then
  sudo chown -R 0:0 "${TFTP_DIR}"
  sudo chown 0:0 "${NFS_ROOT}"
else
  docker run --rm \
    -v "${ROOT_DIR}:/work" \
    debian:bookworm-slim \
...
if [[ "$(stat -c '%u:%g' "${TFTP_DIR}")" != "0:0" || \
  ...
  echo "Use privileged rootful Docker or rerun with passwordless sudo available." >&2
  exit 1
fi
